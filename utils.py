...
class Masking_Loss(nn.Module):
    ...
    def forward(self, logits, target, mask):
        ...
        """
        input = logits.contiguous().view(logits.shape[0]*logits.shape[1], -1) # (N*seq_len, number of classes)
        target, mask = target[:, 1:], mask[:, 1:]
        loss = self.loss_fn(input,
                            target.contiguous().view(-1)) # output in form of scalar
